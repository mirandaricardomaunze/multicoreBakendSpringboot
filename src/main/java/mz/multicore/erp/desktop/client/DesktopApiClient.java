...
public class DesktopApiClient {
    ...
    public byte[] postForBytes(String path, Object body) {
        HttpRequest httpRequest = request(path).header("Accept", "application/pdf")
                .header("Content-Type", "application/json")
                .POST(body == null ? HttpRequest.BodyPublishers.noBody()
                        : HttpRequest.BodyPublishers.ofString(writeJson(body))).build();
        try {
            HttpResponse<byte[]> response = httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofByteArray());
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new ApiClientException(response.statusCode(), "O servidor recusou o pedido de documento.");
            ...
            Thread.currentThread().interrupt();
            throw new ApiClientException("A comunicacao com o servidor foi interrompida.", ex);
        } catch (IOException ex) {
            throw new ApiClientException("Nao foi possivel obter o documento do servidor.", ex);
        }
    }
    ...
    private HttpRequest.Builder request(String path) {
        HttpRequest.Builder builder = HttpRequest.newBuilder()
                ...
                .header(mz.multicore.erp.architecture.version.ClientVersionInterceptor.HEADER,
                        mz.multicore.erp.architecture.version.ClientVersion.current());
