...
public final class OrderDetailsDialog {
    ...
    private void show(OrderDTO order) {
        ...
        DefaultTableModel model = new DefaultTableModel(
                new String[]{"Produto", "Lote", "Qtd", "Preço Unit.", "Desc %", "IVA", "Total"}, 0) {
            @Override public boolean isCellEditable(int row, int column) { return false; }
        };
        for (var line : order.lines()) {
            model.addRow(new Object[]{line.productName(), line.batchNumber() == null ? "—" : line.batchNumber(),
                    line.quantity(), line.unitPrice() + " MT",
                    line.discountPercentage() == null ? "0" : line.discountPercentage().toPlainString(),
                    line.taxRate() == null ? "—" : line.taxRate().toPlainString(), line.lineTotal() + " MT"});
        }
